    ...
    with torch.no_grad():
        for inputs, filename in dataloader:
            ...
            for f,l in zip(filename,predicted):
                data.append({'filename':f,'label':int(l)})
    return data
...
def main():
    ...
    validation_dataset = ImageDataset(data_path=testing_images_path, transform=valid_transform)
    ...
    model.load_state_dict(torch.load('./checkpoint/hw1_1_inference.pth'))
