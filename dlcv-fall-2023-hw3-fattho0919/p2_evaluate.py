...
class CLIPScore:
    ...
    def __call__(self, predictions, images_root):
        ...
        for img_name, pred_caption in predictions.items():
            ...
            highest_score_index = score_list.index(max(score_list))
            lowest_score_index = score_list.index(min(score_list))
            total_score += score

        return total_score / len(predictions), highest_score_index, lowest_score_index, max(score_list), min(score_list)
...
def main(args):
    ...
    # CLIPScore
    clip_score, highest_index, lowest_index, highest_score, lowest_score = CLIPScore()(predictions, args.images_root)
